...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User(username=form.username.data, email=form.email.data, password=hashed_password, accesscode=generate_unique_accesscode(), ign=re.sub(r'[^a-zA-Z0-9]', '', form.username.data).upper()[:12])
        db.session.add(user)
        db.session.commit()
